...
# muis być to aktualiowane co każda zmiane na planszy
...
def wykluczenia(plansza):
    for i in range(9):
            ...
            for j in range(9):
                ...
                #--------------------------------------------------------------
                if(len(plansza[i][j]) == 1): plansza[i][j] = int(plansza[i][j])
                #--------------------------------------------------------------

                if(czy_ustalone(plansza,i,j) == False):
                #Jeżeli na danym polu nie ma konkretnej liczby

                    #for liczba in range(9):
                        if(i%3 == 0 and j%3 == 0):
                            # Jesli jest w górnej lewej kratce
                            # TODO wymyslic lepszy sposób
                            temp = 0
                            for konkretny_item in range (len(plansza[i][j])):
                                dana_liczba = plansza[i][j][konkretny_item]
                                for a in range(i,i+3): 
                                    for b in range(j,j+3):
